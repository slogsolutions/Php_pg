...
function route_new_form($errors = [], $old = []) {
    // Only database users (employees) can create proposals
    if (is_admin() && current_user()['id'] === 0) {
        // Hardcoded admin (id=0) cannot create proposals
        http_response_code(403);
        echo 'Admin user must create an employee account to create proposals.';
        exit;
    }
